...
- receipt_number：唯一的交易号 / 订单号，用于去重。按以下优先级取其一：
  交易单号 > 订单编号 > 订单号 > 商户单号 > 发票号码（字符串，或 null）
- vendor：商户 / 店铺 / 收款方名称（如"商户全称""收款方全称"或店铺名；或 null）
- expense_date：交易日期，格式 YYYY-MM-DD。优先取"支付时间"，
  其次"下单时间 / 创建时间 / 交易时间"（或 null）
- amount：实际支付的总金额，仅数字、不含货币符号或负号。
  订单请取"实付 / 实付款 / 合计"，不要取"原价 / 划线价 / 共减 / 优惠"。
