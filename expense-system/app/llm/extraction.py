...
- extra_fields：一个对象（字段名 -> 值），尽量把发票 / 票据上其余**所有**
  可识别的信息都放进来，例如：发票代码、校验码、机器编号、开票日期、
  购买方名称、购买方纳税人识别号、销售方名称、销售方纳税人识别号、
  金额（不含税）、税率、价税合计(小写)、价税合计(大写)、开票人、复核、收款人、
  备注，以及货物或应税劳务名称等明细。请尽可能完整，不要遗漏图片上的字段。
- confidence：你对本次识别正确性的信心，取值 0.0 到 1.0

只返回合法的 JSON。核心字段无法识别时填 null；extra_fields 无内容时填 {}。\
"""
