...
| 📷 **Receipt & payment recognition** | Upload an image → a vision LLM extracts vendor, date, amount, currency, category, **payment method**, tax and a unique receipt/transaction number → you review & confirm before saving. Handles paper invoices **and** Chinese mobile-payment / e-commerce screenshots — WeChat Pay, Alipay, Pinduoduo, JD, Taobao/Tmall — mapping their varied fields (订单编号/交易单号/商户单号 → receipt no., 实付/实付款/合计 → amount, 支付时间 → date) and normalizing negative bill amounts. |
